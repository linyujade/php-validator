...
class FloatValidator extends BaseValidator {
    ...
    protected function checkMinNumber($value) {
        ...
        if (isset($this->options['min'])) {
            ...
            // 如果有最小值约束，并且输入值是有效的float类型，满足输入值大于等于最小约束值，则验证通过
            return $this->checkFloat($value) && floatval($value) >= floatval($this->options['min']);
        }
    ...
    protected function checkMaxNumber($value) {
        ...
        if (isset($this->options['max'])) {
            ...
            // 如果有最大值约束，并且输入值是有效的float类型，满足输入值小于等于最大约束值，则验证通过
            return $this->checkFloat($value) && floatval($value) <= floatval($this->options['max']);
        }
